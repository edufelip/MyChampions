...
test.describe('@flow-atlas complete product flow atlas', () => {
  test('role onboarding and quick student start', async ({ page }, testInfo) => {
    ...
    await page.getByTestId('auth.roleSelection.continueButton').click();
    await capture(page, testInfo, '01-role-onboarding', '02-student-first-value-home', 'student.home.ready');
  });
  ...
  test('professional onboarding and specialty verification', async ({ page }, testInfo) => {
    ...
    await page.getByTestId('auth.roleSelection.continueButton').click();
    await capture(page, testInfo, '02-professional-onboarding', '01-specialty-selection', 'pro.specialty.screen');
    await page.getByTestId('pro.specialty.add.nutritionist').click();
    await capture(page, testInfo, '02-professional-onboarding', '02-optional-credential-verification', 'pro.specialty.credentialForm');
    await page.getByTestId('pro.specialty.credential.skip').click();
    await capture(page, testInfo, '02-professional-onboarding', '03-active-specialty', 'pro.specialty.row.nutritionist');
    await page.getByTestId('pro.specialty.cta_continue').click();
    await capture(page, testInfo, '02-professional-onboarding', '04-professional-dashboard', 'pro.home.screen');
  });

  test('student daily care and assigned plan tracking', async ({ page }, testInfo) => {
    await chooseRole(page, 'student');
    await capture(page, testInfo, '03-student-daily-care', '01-home-with-assigned-plans', 'student.home.ready');

    await page.goto('/student/nutrition');
    await capture(page, testInfo, '03-student-daily-care', '02-nutrition-tracking', 'student.nutrition.screen');
    ...
    await capture(page, testInfo, '03-student-daily-care', '03-water-intake-logged', 'student.nutrition.waterWidget');

    await page.getByTestId('student.nutrition.planChangeForm.input').fill(
      'Please add another high-protein breakfast option.'
    );
    await capture(page, testInfo, '03-student-daily-care', '04-plan-change-request-ready', 'student.nutrition.planChangeForm');
    await page.getByTestId('student.nutrition.planChangeForm.submitButton').click();
    await capture(page, testInfo, '03-student-daily-care', '05-plan-change-request-submitted', 'student.nutrition.planChangeForm.success');
    ...
    await capture(page, testInfo, '03-student-daily-care', '06-assigned-nutrition-plan', 'pro.nutrition_plan.screen');

    await page.goto('/student/nutrition/plans/e2e-assigned-nutrition-plan/meals/e2e-assigned-meal');
    await capture(page, testInfo, '03-student-daily-care', '07-assigned-meal-detail', 'pro.nutrition_meal.screen');

    await page.goto('/student/training');
    await expect(page.getByTestId('student.training.assignedSessionList')).toBeVisible();
    await capture(page, testInfo, '03-student-daily-care', '08-training-tracking', 'student.training.screen');
    await page.getByTestId('student.training.logBtn-e2e-assigned-session').click();
    await capture(page, testInfo, '03-student-daily-care', '09-workout-session-logged', 'student.training.screen');

    await page.goto('/student/training/plans/e2e-assigned-training-plan');
    await capture(page, testInfo, '03-student-daily-care', '10-assigned-training-plan', 'pro.training_plan.screen');
  });
  ...
  test('student professional connection and QR fallback', async ({ page, context }, testInfo) => {
    ...
    await capture(page, testInfo, '04-student-connections', '01-active-professionals', 'student.professionals.screen');
    await page.getByTestId('student.professionals.unbindButton.0').click();
    await capture(page, testInfo, '04-student-connections', '02-end-relationship-confirmation', 'student.professionals.unbindConfirm');
    ...
    await capture(page, testInfo, '04-student-connections', '03-camera-unavailable-manual-fallback', 'student.professionals.submitError');
    ...
    await capture(page, testInfo, '04-student-connections', '04-invite-pending-confirmation', 'student.professionals.screen');
  });

  test('custom meal create edit quick log share and AI analysis', async ({ page }, testInfo) => {
    await chooseRole(page, 'student');
    await page.goto('/nutrition/custom-meals');
    await capture(page, testInfo, '05-custom-meals', '01-saved-meal-library', 'meal.library.screen');
    ...
    await capture(page, testInfo, '05-custom-meals', '02-quick-log-portion', 'meal.library.quickLog.panel');
    ...
    await capture(page, testInfo, '05-custom-meals', '03-ai-photo-analysis-result', 'meal.library.quickLog.analysis.done');

    await page.goto('/nutrition/custom-meals/e2e-custom-meal');
    await expect(page.getByTestId('meal.builder.field.name.input')).toHaveValue('E2E Recovery Bowl');
    ...
    await capture(page, testInfo, '05-custom-meals', '05-image-upload-complete', 'meal.builder.imageUpload.preview');

    await page.goto('/nutrition/custom-meals/new');
    await capture(page, testInfo, '05-custom-meals', '06-create-custom-meal', 'meal.builder.screen');
  });

  test('shared recipe preview and idempotent save', async ({ page }, testInfo) => {
    await chooseRole(page, 'student');
    await page.goto('/shared/recipes/e2e-shared-recipe');
    await capture(page, testInfo, '06-shared-recipes', '01-shared-recipe-preview', 'shared_recipe.preview');
    await page.getByTestId('shared_recipe.cta.save').click();
    await capture(page, testInfo, '06-shared-recipes', '02-recipient-owned-copy-saved', 'shared_recipe.saved');
  });

  test('professional workbench roster queue and student review', async ({ page }, testInfo) => {
    await chooseRole(page, 'professional');
    await capture(page, testInfo, '07-professional-care-management', '01-task-workbench', 'pro.home.screen');

    await page.getByTestId('tabs.students').last().click();
    await capture(page, testInfo, '07-professional-care-management', '02-student-roster-and-bulk-assignment', 'pro.students.screen');
    ...
    await capture(page, testInfo, '07-professional-care-management', '03-bulk-assignment-selection', 'pro.students.bulk.assignSelected');
    await page.getByTestId('pro.students.bulk.assignSelected').click();
    await capture(page, testInfo, '07-professional-care-management', '04-bulk-plan-picker', 'planPicker.modal');

    await page.goto('/professional/student-profile?studentId=e2e-dual-student');
    await capture(page, testInfo, '07-professional-care-management', '05-student-profile-and-tracking-review', 'pro.student_profile.screen');

    await page.goto('/professional/pending');
    await capture(page, testInfo, '07-professional-care-management', '06-pending-request-queue', 'pro.pending.screen');
    await page.getByTestId('pro.pending.row.0').click({ position: { x: 20, y: 20 } });
    await capture(page, testInfo, '07-professional-care-management', '07-pending-bulk-deny-selection', 'pro.pending.bulkDenyButton');
  });
  ...
  test('professional nutrition and training authoring', async ({ page }, testInfo) => {
    ...
    await captureFlowEvidence(page, testInfo, '08-professional-plan-authoring', '01-nutrition-template-library');
    await page.getByTestId('pro.library.nutrition.create').click();
    await capture(page, testInfo, '08-professional-plan-authoring', '02-new-nutrition-plan-builder', 'pro.nutrition_plan.screen');
    ...
    await captureFlowEvidence(page, testInfo, '08-professional-plan-authoring', '03-training-template-library');
    await page.getByTestId('pro.library.training.create').click();
    await capture(page, testInfo, '08-professional-plan-authoring', '04-new-training-plan-builder', 'pro.training_plan.screen');
    ...
    await capture(page, testInfo, '08-professional-plan-authoring', '05-exercise-search', 'exerciseSearch.modal');
    ...
    await page.getByTestId(/^exerciseSearch\.result\./).first().click();
    await capture(page, testInfo, '08-professional-plan-authoring', '06-exercise-search-detail', 'exerciseSearch.detail');
  });
  ...
  test('professional subscription and specialty management', async ({ page }, testInfo) => {
    ...
    await page.getByTestId('pro.specialty.add.fitness_coach').click();
    await page.getByTestId('pro.specialty.credential.skip').click();
    await capture(page, testInfo, '09-professional-account-controls', '01-specialty-management', 'pro.specialty.screen');
    await page.getByTestId('pro.specialty.remove.nutritionist').click();
    await capture(page, testInfo, '09-professional-account-controls', '02-specialty-removal-assist', 'pro.specialty.removalAssist');
    ...
    await capture(page, testInfo, '09-professional-account-controls', '03-subscription-entitlement', 'pro.subscription.screen');
  });

  test('account privacy language support and logout confirmation', async ({ page }, testInfo) => {
    await chooseRole(page, 'student');
    await page.goto('/settings/account');
    await capture(page, testInfo, '10-account-and-compliance', '01-account-privacy-settings', 'settings.account.screen');

    await page.getByTestId('settings.account.languageRow').click();
    await capture(page, testInfo, '10-account-and-compliance', '02-language-selection', 'settings.languageSelect.screen');
    ...
    await capture(page, testInfo, '10-account-and-compliance', '03-portuguese-account-localization', 'settings.account.screen');

    await page.getByTestId('settings.account.contactRow').click();
    await capture(page, testInfo, '10-account-and-compliance', '04-support-message-dialog', 'settings.account.support.modal');
    await page.getByTestId('settings.account.support.subjectInput').fill('Flow atlas support');
    await page.getByTestId('settings.account.support.bodyInput').fill('Deterministic support flow evidence.');
    await page.getByTestId('settings.account.support.submitCta').click();
    await capture(page, testInfo, '10-account-and-compliance', '05-support-message-sent', 'settings.account.support.success');
    await page.getByTestId('settings.account.support.closeButton').click();

    await page.getByTestId('settings.account.deleteCta').click();
    await capture(page, testInfo, '10-account-and-compliance', '06-delete-account-confirmation', 'settings.account.deleteConfirm');
    await page.getByTestId('settings.account.deleteCancelCta').click();

    await page.getByTestId('settings.account.signOutCta').click();
    await capture(page, testInfo, '10-account-and-compliance', '07-sign-out-confirmation', 'settings.account.signOutConfirm');
  });
  ...
  test('offline read-only recovery surfaces', async ({ page }, testInfo) => {
    ...
    await page.reload();
    await capture(page, testInfo, '11-offline-read-only', '01-student-home-offline', 'student.home.offlineBanner');
    await page.getByTestId('tabs.nutrition').last().click();
    await capture(page, testInfo, '11-offline-read-only', '02-nutrition-write-lock', 'student.nutrition.offlineBanner');
  ...
  test('app modal opens and returns to the role home', async ({ page }, testInfo) => {
    ...
    await capture(page, testInfo, '12-app-shell-modal', '02-return-to-student-home', 'student.home.ready');
  });
});
