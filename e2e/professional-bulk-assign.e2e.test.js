...
describeWithE2EAuthSession('Professional Bulk Assign', () => {
  ...
  it('selects eligible roster rows, chooses a predefined plan, and resets after assignment', async () => {
    ...
    await waitFor(element(by.id('pro.students.bulk.planType.nutrition')))
      .toBeVisible()
      .withTimeout(5000);
    ...
    await waitFor(element(by.id('pro.students.bulk.assignSelected')))
      .toBeVisible()
      .withTimeout(5000);
    ...
    await waitFor(element(by.id('pro.students.bulkAssignToggle')))
      .toBeVisible()
      .withTimeout(5000);
