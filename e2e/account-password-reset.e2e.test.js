...
describeWithE2EAuthSession('Account Password Reset', () => {
  ...
  it('confirms and completes password reset for an email/password user', async () => {
    ...
    await waitFor(element(by.id('settings.account.passwordConfirm'))).toBeVisible().withTimeout(5000);
    await expect(element(by.text('Reset password?'))).toBeVisible();
    await expect(element(by.text("We'll send a password reset link to e2e-auth-session@example.test. Check your inbox after confirming."))).toBeVisible();

    await element(by.id('settings.account.passwordConfirmCta')).tap();
    await waitFor(element(by.id('settings.account.passwordSuccess'))).toBeVisible().withTimeout(5000);
