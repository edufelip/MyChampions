...
const expectedHydrationAfterLogMl = process.env.E2E_HYDRATION_EXPECTED_AFTER_LOG_ML || hydrationLogAmountMl;
const nutritionPlanChangeRequestText =
  process.env.E2E_NUTRITION_PLAN_CHANGE_REQUEST_TEXT || 'Please add more breakfast variety this week.';
...
async function expectAssignedNutritionContent() {
  await waitFor(element(by.id('student.nutrition.macroCard'))).toBeVisible().withTimeout(10000);
...
async function expectHydrationWidget() {
  ...
  await waitFor(element(by.id('student.nutrition.waterWidget.logButton'))).toBeVisible().withTimeout(5000);
}
...
describeWithE2EAuthSession('Student Assigned Nutrition', () => {
  ...
  it('validates and logs hydration intake against the effective goal', async () => {
    ...
    await element(by.id('student.nutrition.waterWidget.intakeInput')).replaceText(hydrationLogAmountMl);
  ...
  it('validates and submits an assigned nutrition plan change request', async () => {
    ...
    await element(by.id('student.nutrition.planChangeForm.input')).replaceText(nutritionPlanChangeRequestText);
