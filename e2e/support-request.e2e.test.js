...
describeWithE2EAuthSession('Support Request', () => {
  ...
  it('validates and submits an in-app support request', async () => {
    ...
    await waitFor(element(by.id('settings.account.support.modal')))
      .toBeVisible()
      .withTimeout(5000);
    ...
    await waitFor(element(by.id('settings.account.support.submitCta')))
      .toBeVisible()
      .withTimeout(5000);
    await new Promise((resolve) => setTimeout(resolve, 500));

    await element(by.id('settings.account.support.submitCta')).tap();
    await waitFor(element(by.id('settings.account.support.subjectError')))
      .toBeVisible()
      .withTimeout(5000);

    await element(by.id('settings.account.support.subjectInput')).replaceText('E2E support issue');
    await element(by.id('settings.account.support.bodyInput')).replaceText(
      'The support request flow should submit safely.',
    );
    await element(by.id('settings.account.support.submitCta')).tap();

    await waitFor(element(by.id('settings.account.support.success')))
      .toBeVisible()
      .withTimeout(10000);
  });
});
