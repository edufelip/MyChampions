...
async function openLanguageSelect() {
  ...
  await waitFor(element(by.id('settings.languageSelect.screen')))
    .toBeVisible()
    .withTimeout(5000);
}
...
describeWithE2EAuthSession('Language Select', () => {
  ...
  it('saves language changes immediately and discards unsaved selections on back', async () => {
    ...
    await waitFor(element(by.id('settings.languageSelect.option.pt-BR.checkmark')))
      .toBeVisible()
      .withTimeout(5000);
    ...
    await waitFor(element(by.id('settings.languageSelect.option.es-ES.checkmark')))
      .toBeVisible()
      .withTimeout(5000);
    ...
    await waitFor(element(by.id('settings.languageSelect.option.en-US.checkmark')))
      .toBeVisible()
      .withTimeout(5000);
    await element(by.id('settings.languageSelect.saveButton')).tap();
    await waitFor(element(by.id('settings.account.screen')))
