...
test('mobile CI workflows install dependencies without Actions-backed caches', () => {
  ...
  for (const workflowFile of workflowFiles) {
    ...
    assert.equal(workflow.includes('package-lock.json'), false, `${workflowFile} should not reference package-lock.json`);
  }
});
...
test('mobile package scripts do not route through npm-run wrappers', () => {
  ...
  for (const [scriptName, command] of Object.entries(packageJson.scripts ?? {})) {
    assert.equal(command.includes('npm run'), false, `${scriptName} should call Yarn scripts directly`);
  }
});
...
test('mobile unit tests include root-level and nested feature test files', () => {
  ...
  assert.match(unitTestScript, /features\/\*\.test\.ts/, 'test:unit should include features/*.test.ts');
  assert.match(unitTestScript, /features\/\*\*\/\*\.test\.ts/, 'test:unit should include nested feature tests');
});
