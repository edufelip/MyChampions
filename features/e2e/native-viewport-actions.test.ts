...
const professionalTrainingBuilderSource = readSource(
...
const offlineAccountWriteLockSource = readSource('e2e/offline-account-write-lock.e2e.test.js');
...
const planMetadataFormSource = readSource('features/plans/components/PlanMetadataForm.tsx');
const planPickerModalSource = readSource('components/ds/patterns/PlanPickerModal.tsx');
const professionalBulkAssignSource = readSource('e2e/professional-bulk-assign.e2e.test.js');
const professionalStudentProfileSource = readSource('e2e/professional-student-profile.e2e.test.js');
...
test('professional specialty stories scroll the credential action into view', () => {
  ...
  for (const path of specialtyConsumerPaths) {
    ...
    assert.doesNotMatch(source, /element\(by\.id\('pro\.specialty\.credential\.skip'\)\)\.tap\(\)/);
  }
...
test('professional plan pickers use semantic assignment controls across native viewports', () => {
  assert.match(planPickerModalSource, /testID=\{`planPicker\.assign\.\$\{plan\.id\}`\}/);
  assert.match(planPickerModalSource, /onShow=\{\(\) => setIsPresented\(true\)\}/);
  assert.match(planPickerModalSource, /if \(!isVisible\) \{\s+setIsPresented\(false\);\s+\}/);
  assert.match(
...
test('account settings scrolls compact legal and support controls before interaction', () => {
  ...
  assert.match(accountSettingsSource, /require\('\.\/account-settings-actions'\)/);
