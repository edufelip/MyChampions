...
export const MAX_COMPRESSED_PHOTO_BYTES = 1_500_000;
...
export class BrowserPhotoTooLargeError extends Error {
  readonly code = 'file_too_large';

  constructor() {
    super('Compressed photo exceeds 1.5 MB');
    this.name = 'BrowserPhotoTooLargeError';
  }
}
...
export async function compressBrowserPhotoWithinLimit(
  ...
  for (const attempt of COMPRESSION_ATTEMPTS) {
    const blob = await encode({
      width: Math.max(1, Math.round(size.width * attempt.scale)),
      height: Math.max(1, Math.round(size.height * attempt.scale)),
  ...
  throw new BrowserPhotoTooLargeError();
}
...
async function compress(photo: PickedPhoto): Promise<Blob> {
  try {
    ...
    return await compressBrowserPhotoWithinLimit(
      size,
