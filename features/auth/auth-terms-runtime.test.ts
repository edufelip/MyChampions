...
const authTermsE2ESource = readFileSync(join(process.cwd(), 'e2e/auth-terms.e2e.test.js'), 'utf8');
...
test('auth terms launches one fresh native app instance per case', () => {
  ...
  assert.equal(authTermsE2ESource.match(/device\.launchApp/g)?.length, 1);
});
