...
export const DEFAULT_TERMS_URL = 'https://portfolio.eduwaldo.com/projects/my-champions/terms_of_use';
export const DEFAULT_PRIVACY_POLICY_URL = 'https://portfolio.eduwaldo.com/projects/my-champions/privacy_policy';
...
export function resolveTermsUrl(value: string | null | undefined, fallback = DEFAULT_TERMS_URL): string {
  if (!value) return fallback;
  const next = value.trim();
  return next.length > 0 ? next : fallback;
}

export function resolvePrivacyPolicyUrl(value: string | null | undefined, fallback = DEFAULT_PRIVACY_POLICY_URL): string {
  if (!value) return fallback;
  const next = value.trim();
  return next.length > 0 ? next : fallback;
}
