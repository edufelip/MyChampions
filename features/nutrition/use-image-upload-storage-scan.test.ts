...
test('image upload logic and source tests do not encode legacy storage error codes', () => {
  for (const relativePath of [
    ...
    assert.equal(
      source.includes(legacyQuotaCode),
      false,
      `${relativePath} still contains legacy quota code`,
    );
    assert.equal(
      source.includes(legacyAuthCode),
      false,
      `${relativePath} still contains legacy auth code`,
    );
    assert.equal(
      source.includes(providerSpecificCodePhrase),
      false,
      `${relativePath} still describes provider-specific codes`,
    );
  }
});

test('mobile image upload source does not build client-owned user storage paths', () => {
  for (const relativePath of ['image-upload-source.ts', 'use-image-upload.ts']) {
    const source = readFileSync(join(__dirname, relativePath), 'utf8');

    assert.equal(
      source.includes('users/${'),
      false,
      `${relativePath} should not interpolate client-owned user paths`,
    );
    assert.equal(
