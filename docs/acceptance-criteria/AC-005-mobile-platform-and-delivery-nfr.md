...
- `AC-512`: Core screens meet accessibility baseline for contrast, dynamic text scaling, focus order, and screen-reader labels. Each workout session exposes one dedicated expandable-details control with a minimum 44×44 CSS px touch/focus target, a localized label, and its expanded/collapsed state.
