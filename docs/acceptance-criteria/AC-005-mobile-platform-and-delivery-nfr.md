...
- `AC-516`: Feature-aware test impact is resolved from a checked-in ownership/suite manifest, the PR merge-base-to-head change set, reverse feature dependencies, and base/head TypeScript import consumers. Feature-only changes select only their affected Playwright/Detox suites; shared or indirect changes widen selection; renames, copies, and deletions preserve both historical and current ownership; invalid, unknown, oversized, or unresolved changes select the complete registered CI matrix. Shadow mode must retain the existing authoritative PR workflows until the documented evidence window passes.
