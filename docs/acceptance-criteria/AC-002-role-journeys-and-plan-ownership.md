...
- `AC-266`: After successful sign-in or create-account, users are routed to a terms-acceptance gate and cannot proceed to role-selection or role-home until the required terms version is accepted. While pending, only the exact configured terms URL reached with the `terms` legal intent may bypass the gate; malformed, repeated, unsafe, or arbitrary approved-origin WebView parameters are rejected and recover to the gate. Exact configured legal URLs may use a trusted HTTPS host outside the default origin without widening arbitrary route access.
