...
- `.github/workflows/pr-selective-tests.yml` is the self-hosted, source-free
  ...
  run candidate code. It runs on the Linux self-hosted runner. Pull-request base filters cover `main`,
  ...
  Its externally visible required-check name remains `Hosted candidate preflight`
  for branch-protection compatibility; the name does not select GitHub-hosted
  capacity.
- `.github/workflows/trusted-selective-tests.yml` is the authoritative
  ...
  protected default branch `main`. Its self-hosted authorization job validates the
  triggering run against the live pull-request API before candidate checkout or
  credentialed workload. Merge-group authorization validates every associated
  ...
  A PR targeting `release/**`/`hotfix/**` no longer force-selects the
  complete matrix by virtue of its base branch alone (CI web-primary
  redesign, Step 7); only an explicit `ci:full` label does, same as any
  other base branch. Full Detox validation for release/hotfix branches now
  comes exclusively from `detox-protected-full.yml`'s `push` trigger on
  those branches — see that workflow's row below.
...
- `.github/workflows/firebase-distribution-android.yml` and
  `firebase-distribution-ios.yml` are development-only Firebase App Distribution
  workflows. Each runs on a `develop` push (including a merged pull request) or
  manual dispatch, builds the `dev` identity, sends it to the `base-group` tester
  group, and fails closed when `ENV_FILE` has a missing or production API URL.
  Android uses the Linux/Android self-hosted label and iOS uses the macOS/iOS
  self-hosted label.
- The repository variable `MYCHAMPIONS_ENABLE_IOS_TESTS` is the default-on
  ...
  manual iOS smoke lane, the protected full iOS Detox lane, and
  provider-backed iOS Test Store validation (there is no selected iOS
  Detox lane on the PR path any more — Detox left the PR path entirely in
  the CI web-primary redesign, Step 7); an unset variable
...
- Native builds no longer require Firebase config files
  (`google-services.json`, `GoogleService-Info*.plist`) or Firebase project files
  after the local-server migration. The development distribution workflows use a
  Firebase service account only to upload an already-built binary; they do not
  restore a Firebase app runtime dependency.
...
- Promotion target (pending verification): the self-hosted, source-free PR/merge-group
  preflight runs on the Linux self-hosted label without checking out candidate code. The protected-`main`
  ...
  candidate checkout or credentialed workload. Merge-group authorization
  ...
  the trusted workflow is never loaded from those branches, and a
  release/hotfix base no longer forces the complete matrix on its own
  (Step 7 of the CI web-primary redesign) — only an explicit `ci:full`
  label does, same as any other base.
- Promotion target (pending verification): before the PR preflight completes,
  ...
  event.
- Promotion target (pending verification): candidate jobs have only
...
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------- | ------------------------------------------------------------- |
| `ENV_FILE`                            | `trusted-selective-tests` native lanes; manual `android-pr`/`ios-pr`; Android/iOS release; Firebase distribution | Trusted selective native lanes write secret bytes only to a validated per-job mode-`0600` regular file below `$RUNNER_TEMP` and expose workspace `.env` as an absolute symlink to that target; legacy/release/distribution paths write mode-`0600` temporary files and remove them after the job. Firebase distribution requires a non-production server target and matching development public OAuth/RevenueCat values. | Raw multiline `.env` content                    | Yes for native/full-matrix and release/distribution execution |
...
| `FIREBASE_SERVICE_ACCOUNT_JSON`       | Firebase distribution                                                                                            | Dedicated Firebase App Distribution Admin upload auth; written only to a temporary mode-`0600` credential file and deleted after the job                                                                                                                                                                                                                                                                                 | Raw JSON string for Firebase service account    | Yes                                                           |
| `FIREBASE_APP_ID_ANDROID_DEV`         | `firebase-distribution-android`                                                                                  | Registered Firebase App ID for `com.edufelip.mychampions.dev` Android distribution                                                                                                                                                                                                                                                                                                                                       | Firebase Android App ID                         | Yes                                                           |
| `FIREBASE_APP_ID_IOS_DEV`             | `firebase-distribution-ios`                                                                                      | Registered Firebase App ID for `com.edufelip.mychampions.dev` iOS distribution                                                                                                                                                                                                                                                                                                                                           | Firebase iOS App ID                             | Yes                                                           |
| `IOS_TEAM_ID`                         | `ios-release`; `firebase-distribution-ios`                                                                       | Apple Developer Team ID for xcodebuild signing                                                                                                                                                                                                                                                                                                                                                                           | Apple Team ID string                            | Yes                                                           |
...
| `IOS_ADHOC_KEYCHAIN_PASSWORD`         | `firebase-distribution-ios`                                                                                      | Temporary keychain password for development IPA signing                                                                                                                                                                                                                                                                                                                                                                  | Plain string                                    | Yes                                                           |
| `IOS_ADHOC_CERT_P12_BASE64`           | `firebase-distribution-ios`                                                                                      | Apple Distribution certificate import for the Ad Hoc development IPA                                                                                                                                                                                                                                                                                                                                                     | Base64 `.p12`                                   | Yes                                                           |
| `IOS_ADHOC_CERT_PASSWORD`             | `firebase-distribution-ios`                                                                                      | Apple Distribution certificate password                                                                                                                                                                                                                                                                                                                                                                                  | Plain string                                    | Yes                                                           |
| `IOS_ADHOC_PROFILE_BASE64`            | `firebase-distribution-ios`                                                                                      | Ad Hoc provisioning profile for `com.edufelip.mychampions.dev`, with tester device UDIDs registered                                                                                                                                                                                                                                                                                                                      | Base64 `.mobileprovision`                       | Yes                                                           |
| `IOS_ADHOC_PROFILE_NAME`              | `firebase-distribution-ios`                                                                                      | Ad Hoc provisioning profile specifier                                                                                                                                                                                                                                                                                                                                                                                    | Plain string                                    | Yes                                                           |
...
| Trusted workflow authorization                      | `trusted-selective-freshness`, `pr-selective-tests`, and `trusted-selective-tests` | Protected-`main` freshness invalidates reusable success for owner-authored same-upstream PRs without candidate checkout; the self-hosted, source-free PR preflight uses `statuses: read` to await the pending description for its canonical exact-event fingerprint; the protected-default-branch `workflow_run` validates triggering-run and live-PR provenance before candidate checkout/credentialed workload. A release/hotfix base no longer forces full on its own (only an explicit `ci:full` label does, since the CI web-primary redesign's Step 7 — Detox left the PR path and full Detox validation for those branches now comes exclusively from `detox-protected-full.yml`'s `push` trigger), merge groups validate every associated live PR, and candidate jobs have only `contents: read`. Evidence pending. |
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `trusted-selective-freshness.yml`   | Protected-`main`, self-hosted, source-free owner/upstream `pull_request_target` metadata invalidator; no candidate checkout                                                                                                                                                                                                                                                                                                                                                                                                                 | None                                                                                                                                                                                                                                                           |
| `pr-selective-tests.yml`            | self-hosted, source-free `pull_request` preflight for `main`, `release/**`, and `hotfix/**`, plus future-compatible `merge_group`; never checks out candidate code or targets self-hosted labels; uses `statuses: read` to await trusted freshness pending for its canonical exact-event fingerprint                                                                                                                                                                                                                                        | None                                                                                                                                                                                                                                                           |
| `trusted-selective-tests.yml`       | Protected-`main` authoritative workflow; accepts authorized `workflow_run`, direct `main` push, and `workflow_dispatch` at ref `main` with a live-resolved PR number and forced full selection; push publishes no PR gate; has no direct `merge_group`, release-branch, or hotfix-branch trigger; a release/hotfix base no longer forces full on its own (only an explicit `ci:full` label does — Step 7 of the CI web-primary redesign removed `detox-ios-selected`/`detox-android-selected` and the base-branch auto-broadening entirely) | No repository secret at all — the only self-hosted lane left (`web-selected`) needs none; `ENV_FILE` was only ever consumed by the now-deleted selected iOS/Android Detox jobs                                                                                 |
...
| `detox-protected-full.yml`          | Manual/release-published/protected-branch-push test-only full Detox validation; also triggers on `push` to `release/**`/`hotfix/**` (Step 6 of the CI web-primary redesign — no separate authorization job, since push events to those protected branches can't be spoofed or come from a fork, the same trust basis `android-release.yml`/`ios-release.yml` already rely on); its iOS job is skipped for new runs when `MYCHAMPIONS_ENABLE_IOS_TESTS=false`, while Android remains enabled                                                 | `ENV_FILE`                                                                                                                                                                                                                                                     |
...
| `firebase-distribution-android.yml` | `develop` push/manual development APK build and Firebase App Distribution upload; uses the checked-in debug certificate only with `-PdevReleaseWithDebugSigning=true`, never for `productionRelease`                                                                                                                                                                                                                                                                                                                                        | `ENV_FILE`, `FIREBASE_SERVICE_ACCOUNT_JSON`, `FIREBASE_APP_ID_ANDROID_DEV`                                                                                                                                                                                     |
| `firebase-distribution-ios.yml`     | `develop` push/manual development IPA build and Firebase App Distribution upload                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `ENV_FILE`, `FIREBASE_SERVICE_ACCOUNT_JSON`, `FIREBASE_APP_ID_IOS_DEV`, `IOS_ADHOC_KEYCHAIN_PASSWORD`, `IOS_ADHOC_CERT_P12_BASE64`, `IOS_ADHOC_CERT_PASSWORD`, `IOS_ADHOC_PROFILE_BASE64`, `IOS_ADHOC_PROFILE_NAME`, `IOS_TEAM_ID`                             |
...
| iOS           | `self-hosted, macOS, ARM64, mychampions-ci, mychampions-ios`    | Repository-scoped Apple Silicon runner; Xcode 26 with iOS SDK 26+; disposable owned `iPhone 17` simulator UDID; CocoaPods; Homebrew path; AppleSimulatorUtils (`applesimutils`) for Detox | Existing registration and toolchain evidence is capacity evidence only; D-195 owner/trust/action/environment/disposable-UDID gates and exact-head simulator proof remain pending                                                                                            |
...
protected-default-branch trusted workflow and its self-hosted triggering-run
and live-PR validation before candidate checkout or credentialed workload.

The `mychampions-web-only` companion is the deliberate exception to the
...
exception is controlled by the trusted workflow's routing change and is rolled
back by restoring the shared web/Android concurrency group if either lane
regresses by more than 15 percent.
...
6. Prove the protected-`main` freshness workflow registration and one live
   exact-event fingerprint handshake with the Source-free preflight,
   self-hosted, source-free preflight, protected-`main` trusted workflow
