...
| Backlog Item | Status | FR | UC | AC | BR | TC |
|---|---|---|---|---|---|---|
| BL-005 Student plan change request flow | Implemented — screens and MyChampions server plan-change request endpoints are wired; professional notification delivery remains deferred until push infrastructure exists | FR-211 | UC-002.13 | AC-255 | BR-269 | TC-259 |
| BL-006 Professional starter template library | Implemented for local flow — pure logic layer, server-backed predefined plan reads/assignment paths, and focused source tests are in place; durable starter-template authoring remains tracked for a later server route | FR-212 | UC-002.14 | AC-256 | BR-270 | TC-260 |
...
| BL-009 Subscription pre-lapse warning | Partial — warning UI and copy exist, but an authoritative expiry-risk signal is still required. Student capacity no longer masquerades as billing expiry, and entitlement locks no longer block recovery actions. | FR-215 | UC-002.15 | AC-312 | BR-273 | TC-311 |
...
| Backlog Item | Status | FR | UC | AC | BR | TC |
|---|---|---|---|---|---|---|
| BL-016 Feature-aware selective test execution | In progress — the manifest, fail-closed dependency resolver, typed executor/profile contracts, scenario-isolated native fixtures, dedicated Metro routing, Android preboot/cleanup, both-platform feature selection, full safety fallbacks, registered Mac/WSL capacity, shared-host locks, zero-success-artifact policy, contract tests, all-external approval, sole-collaborator roster, read-only default workflow token, disabled workflow PR approval, selected-actions allowlist, and full-SHA pinning are implemented or have dated setting evidence. Promotion remains blocked by D-195: protected-`main` hosted freshness invalidation, a GitHub-hosted-only PR/future-merge-group preflight that awaits that invalidation, protected-`main` `workflow_run` execution, unique owner/upstream live-PR authorization before candidate checkout and final publication, read-only candidate/self-hosted job tokens, globally serialized owned pending/final status publication, exact backend SHA, executable and live proof of the runner-temp secret-target/workspace-link lifecycle, cancellation-safe durable device ownership in a separate hardened persistent ledger, cleanup-failure record retention, next-run exact stale-resource recovery, required `main` strict-up-to-date/PR/`Hosted candidate preflight`/`Selective CI gate`/conversation/admin rules, and a complete exact-head web/iOS/Android matrix all require verified evidence before the workflow is authoritative or merge-ready. The personal public repository cannot enable merge queue, so checked-in `merge_group` support is future-compatible. Static labels remain targetable by any approved workflow; adding collaborators or approving external workflow changes first requires pausing runners and adopting private-broker/JIT/ephemeral isolation. | FR-271, FR-272 | N/A | AC-540, AC-542 | BR-256, BR-344 | TC-518, TC-519 |
| BL-017 On-demand manual QA Skill + Linear runs | Implemented — global Skill + MyChampions family adapter covering `mobile`/`web`, `api`, `food`, `exercises`; smoke packs + Linear QA Runs (D-199). Native mobile Skill pass and VM `dev` API/DB remain Pending. | N/A | UC-001 (smoke) | N/A | D-199 | qa-smoke-pack, qa-manual-run-playbook, server/food/exercises qa-smoke-pack |
...
| BL-106 Named predefined plans + clone/bulk assignment | Implemented — screens use MyChampions server plan-list/predefined assignment paths and server-backed food search against the mirrored local catalog; SC-207 keeps its complete add-food editor in measured scroll flow on compact native viewports | FR-223, FR-224, FR-225, FR-226, FR-242 | UC-002.20 | AC-264, AC-265, AC-541 | BR-281, BR-282, BR-283, BR-343 | TC-268, TC-269, TC-270, TC-277 |
| BL-108 AI meal photo macronutrient analysis | Implemented locally — logic/hook/source/camera/compression use the MyChampions server analyzer boundary; provider configuration and durable remote media storage remain future server/provider work | FR-229, FR-230, FR-231, FR-232, FR-233, FR-234, FR-235, FR-236, FR-237, FR-238, FR-239 | UC-003.9 | AC-513, AC-514, AC-515, AC-516, AC-517, AC-518, AC-519 | BR-286, BR-287, BR-288, BR-289, BR-290 | TC-271, TC-272, TC-273, TC-274, TC-285 |
