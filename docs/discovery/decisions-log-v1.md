...
- `D-087`: Role-selection UI and quick self-guided entry are implemented with Data Connect-backed role-lock persistence/session wiring; selected role now routes to concrete journeys (`student` -> `/`, `professional` -> `/professional/specialty`).
