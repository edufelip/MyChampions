...
- `D-106`: AI meal photo analysis uses OpenAI GPT-4o Vision via a Firebase Cloud Function proxy (`analyzeMealPhoto`). The OpenAI API key is server-side only; no key is shipped in the client binary or exposed via client-accessible environment variables.
