...
- `Done`: Focused iOS Detox manual smoke wiring added. `.detoxrc.js` supports `DETOX_JEST_CONFIG`, `e2e/jest.smoke.config.js` selects the migration-critical smoke specs, `package.json` exposes `test:e2e:ios:debug:smoke`, and the legacy manual-only `.github/workflows/ios-pr.yml` reconciles any legacy `wix/brew` AppleSimulatorUtils (`applesimutils`) installation to the supported `wix-incubator/brew` formula before one workflow-owned Debug Detox build and the two fresh-Metro test phases with `DETOX_SKIP_BUILD=true` when dispatched.
...
- `In progress`: ET-201 support abuse controls have local server-route and browser evidence, plus production Nginx templates. Applying the ingress configuration and performing a controlled production `429` verification remain `Pending` until explicit production-change approval; no local test result is treated as production enforcement proof.
...
- `Done`: Firebase App Distribution is a delivery-only development lane. `.github/workflows/firebase-distribution-android.yml` and `firebase-distribution-ios.yml` run on every `develop` push (including a merge) or manual dispatch, build only `MyChampions Dev`, and upload to `base-group` without native Firebase config injection. A dedicated Firebase App Distribution Admin CI identity and repository app-ID/config secrets are configured; `ENV_FILE` targets the isolated Dev API rather than production. Hosted run `34766495807` delivered Android `1.0.0-dev (4)`, and manual-dispatch run `34769384751` delivered iOS `1.0.0 (11)`; Firebase shows two invited `base-group` testers for each release. Tester installation/device behavior remains separate evidence and is not established by these provider receipts.
...
- `In progress (D-195 promotion gate)`: The complete persistent-runner security and repository-enforcement boundary requires the trusted-workflow and live evidence below.
  - Keep `trusted-selective-freshness.yml` as a protected-`main`,
    self-hosted, source-free `pull_request_target` metadata workflow with no candidate
    ...
    `merge_group`, self-hosted, source-free; its pull-request job has only
...
- `Done`: Local mobile auth and server-backed source modules now use the MyChampions server or explicit E2E fixtures. Missing local server URL/auth fails closed outside those fixtures.
  ...
  - Done (ET-71): The client-side gap this checklist previously left implicit — no unauthenticated forgot/reset-password UI and no deep-link consumer for the debug-outbox `mychampions://auth/password-reset?token=...&email=...` link — is closed. `app/auth/forgot-password.tsx` (SC-226) and `app/auth/password-reset.tsx` (SC-227, routed to match the deep-link path) call `requestPasswordResetFromSource()` and the new `confirmPasswordResetFromSource()` against the server's `POST /auth/password-reset` and `POST /auth/password-reset/confirm` endpoints respectively. See `D-203` in `decisions-log-v1.md`.
  - Done (ET-74): The server's `POST /auth/password-reset/confirm` endpoint that ET-71's client above depends on is merged and available, so the full request→confirm flow is wired end to end with no outstanding backend dependency.
  ...
  - Done: Legacy Firebase Cloud Functions project files, Firestore rules/index files, Firebase project aliases, and native Firebase runtime configuration are removed from the mobile package. Firebase App Distribution is retained only as a CI delivery transport for development binaries (D-056); it does not restore those retired runtime surfaces.
...
- `Done`: Fresh SC-207/SC-208 professional builder initialization runs after the route-scope reset, preserves Ready, and exposes the first Add meal/Add session action on mobile web at 390x844 and 320x720 (ET-94, D-213, TC-275A, TC-278A).
