...
- `Done`: Focused iOS Detox manual smoke wiring added. `.detoxrc.js` supports `DETOX_JEST_CONFIG`, `e2e/jest.smoke.config.js` selects the migration-critical smoke specs, `package.json` exposes `test:e2e:ios:debug:smoke`, and the legacy manual-only `.github/workflows/ios-pr.yml` installs AppleSimulatorUtils (`applesimutils`) before running the debug Detox build/test commands when dispatched.
