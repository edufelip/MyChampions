...
- `In progress`: Firebase App Distribution is restored as a delivery-only development lane. `.github/workflows/firebase-distribution-android.yml` and `firebase-distribution-ios.yml` run on every `develop` push (including a merge) or manual dispatch, build only `MyChampions Dev`, and upload to `base-group` without native Firebase config injection. Live activation remains pending a dedicated Firebase App Distribution Admin service account, repository app-ID secrets, valid iOS Ad Hoc signing assets and registered tester UDIDs, and a non-production server URL in `ENV_FILE`; the dedicated dev API/DB prerequisite remains tracked above.
