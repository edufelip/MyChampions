...
- `Done`: `features/nutrition/meal-photo-analysis.logic.ts` — mobile pure functions: `isValidMacroEstimate`, `parseMacroEstimateFromResponse`, `mapMacroEstimateToMealInput`, `normalizePhotoAnalysisError`. `PhotoAnalysisErrorReason` union includes `'unauthenticated'` (added in code-review session). Analyzer prompt construction is server-owned in `server/src/nutrition/meal-photo-analysis-prompt.ts`.
...
- `Done`: `use-subscription.ts` — `aiEntitlementStatus` state, `hasAiAccess` derived bool, and role-aware `openAiUpgradePaywall(lockedRole)` action; students resolve the guarded student offering, professionals resolve `default_professional`, and missing roles fail closed; a single `getCustomerInfo()` call maps both entitlements; `RevenueCatUI.presentPaywall` is the production dep (D-190).
...
- `Partial`: The live 2026-07-23 RevenueCat audit corrected the App Store identifiers to `professional_annual`, `professional_monthly`, `student_annual`, and `student_monthly`. App Store production products exist but report missing metadata; Play Store has no products; the development Android app configuration is absent; and the shared generic Test Store products are not attached to entitlements.
- `Done`: RevenueCat entitlement `professional_pro` is attached to `professional_annual`, `professional_monthly`, and `professional_test`; `student_pro` is attached to `student_annual`, `student_monthly`, and the existing legacy-typo `student_text` development product.
- `Partial`: Offerings `default_student` and `default_professional` each have annual/monthly packages, but neither has a configured paywall, no Android products are mapped, the same unattached generic Test Store yearly/monthly products are shared across both plans, and `default_student` monthly incorrectly maps the development `professional_test` product.
...
- `Done`: `presentAiPaywall` accepts the guarded student offering (`default_student` normally; temporary `test_student` only for explicit dev/Test Store config); `presentProPaywall` uses `PRO_OFFERING_ID` (`'default_professional'`). Production `presentPaywall` resolves the requested offering object via `Purchases.getOfferings().all[offeringIdentifier]` (D-152, D-190).
...
- `Pending (provider approval/permission)`: Create four plan-specific Test Store products and attach/map them independently; correct the student monthly development package; add production Android products and the missing development Android app/products; optionally publish branded paywalls instead of the SDK default; create the first server-only secret API key; restrict sandbox entitlement access after live verification; deploy the canonical reconciler and send a signed test event. The current dashboard reports that this account cannot add app configurations, so the Android dev app may require an owner/billing-plan permission change.
