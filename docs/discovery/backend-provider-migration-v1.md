...
| Acceptance criteria                     | `docs/acceptance-criteria/AC-005-mobile-platform-and-delivery-nfr.md`        | Superseded | Current acceptance criteria no longer require mobile-owned Firebase runtime config or storage surfaces. Firebase App Distribution remains a CI-only delivery transport for `dev` binaries, not an app-domain provider dependency. |
