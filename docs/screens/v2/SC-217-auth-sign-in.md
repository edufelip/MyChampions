...
- New-password validation reuses the same policy as account creation (`isPasswordPolicySatisfied` from `create-account.logic.ts`): 8+ characters, uppercase, number, ASCII symbol, no emoji.
