...
| TC-437 | Missing/Deleted/Unauthorized Edit ID Fails Closed (ET-100, SC-214) | Authenticated user opens `/nutrition/custom-meals/:mealId` with a `mealId` that does not resolve (stale, deleted, mistyped, or unauthorized) | Wait for the route to settle and inspect the rendered screen | The screen never renders a blank/editable create-looking form; it shows a semantic error state (`meal.builder.error`) with localized copy, a Retry action, and a Back to recipes action; Save and Share are absent until a real meal loads; `/nutrition/custom-meals/new` create mode is unaffected |
| TC-438 | Same-Instance Edit→Create Transition Clears Stale State (ET-100, SC-214) | Authenticated user's mounted meal builder screen instance already hydrated a real edit target (`mealId` resolved) | Without unmounting the screen, transition the same instance to `/nutrition/custom-meals/new` (e.g. `router.replace`) | The form fields, saved meal id, validation/save errors, attach-photo toggle, AI photo-analysis state, and image-upload state from the previously hydrated meal are all cleared; the create form shows an empty draft and does not carry over the prior meal's data or share id |
