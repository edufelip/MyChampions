...
| Professional student roster hero responsive | `yarn playwright test --config=playwright.professional-students-roster.config.ts e2e/web/professional-students-roster-responsive.spec.ts` | Pixel 5 mobile Chromium, en-US | Small-phone roster heading regression (ET-159) | Authenticated professional roster fixture verifies no horizontal overflow and that the "My students" hero title column is wide enough to wrap only between whole words at 320 CSS px (the "Bulk assign plan" CTA moves to its own row below the title instead of sharing it), while the existing 390 CSS px layout (CTA inline with the title) is unchanged. Evidence is local/manual unless selected by the feature-impact runner. |
