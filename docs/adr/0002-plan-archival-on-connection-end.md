...
**Consequences:**
- `endConnection` in `connection-source.ts` calls the MyChampions server endpoint for local bearer sessions; the server connection repository archives the assigned plan for the relevant Specialty and restores the most recent self-managed plan associated with the ending Connection.
- The archival + restore must run inside the same server/Postgres lifecycle transaction as the connection end to avoid partial state.
