...
When a Connection ends (by either party), the assigned plan for that Specialty is automatically archived. The Student's most recent self-managed plan for that Specialty (if one exists) becomes the Effective Plan. If no self-managed plan exists the Student enters the empty-state — no scaffold is auto-generated.
...
**Consequences:**
- `endConnection` in `connection-source.ts` currently only sets `status: 'ended'`. It must be extended to also archive the assigned plan for the relevant Specialty and restore the most recent self-managed plan.
