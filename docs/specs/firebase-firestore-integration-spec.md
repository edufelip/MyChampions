...
- `connections/{connectionId}`: student-professional lifecycle state. Pending Connection creation from invite submission is performed through the authenticated `submitInviteCode` Cloud Function; direct client Connection creates are denied by Firestore rules.
- `connectionInviteGuards/{professionalUid}_{studentUid}_{specialty}`: deterministic backend-created guard for a same Student/Professional/Specialty pending invite request. It is deleted only as part of the transaction that moves the referenced pending Connection to `active` or `ended`.
- `professionals/{professionalUid}/pendingStudents/{studentUid}` and `professionals/{professionalUid}/pendingStudentSlots/{slotId}`: deterministic pending-queue occupancy state. The backend allocates one of ten fixed slots for each unique pending Student per Professional, reuses that slot when the same Student has both Specialty requests pending, and rejects new unique Students when all ten slots are occupied. Client lifecycle transactions may clear occupancy only when the referenced pending Connection leaves `pending_confirmation`.
...
- The `submitInviteCode` Cloud Function enforces pending duplicate/cap invariants by reading and writing deterministic guard, pending-student, and pending-slot documents in the same Firestore transaction as the pending Connection create. This avoids query-then-random-document races under concurrent invite submissions.
- Specialty removal is performed through the authenticated `removeProfessionalSpecialty` Cloud Function, not direct client Firestore mutation. The Function validates the caller owns the Specialty, checks no active/pending Connections exist for that Specialty, deletes the scoped invite code and lookup, then deletes `specialties/{professionalUid}_{specialty}` with Admin privileges. Firestore rules deny direct client Specialty deletion and direct `isActive=false` updates.
...
- Plan lifecycle archive/restore writes set `lifecycleConnectionId` on the affected `nutritionPlans/{planId}` or `trainingPlans/{planId}` document. Firestore rules use that marker to validate the exact `connections/{connectionId}` before/after transition with `get()` and `getAfter()`, or the already-ended Connection for safe post-end reconciliation.
...
- Connection end, whether initiated by Professional or Student, archives assigned plans for the ending Connection and restores the latest Self-Managed Plan tied to that Connection when one exists. No replacement plan is auto-created. Firestore rules also permit the same archive/restore writes after the Connection is already ended so a partially completed client lifecycle can be reconciled without reopening access; assigned plan reads still require active matching access.
