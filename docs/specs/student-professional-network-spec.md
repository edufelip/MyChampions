...
11. Student submits invite code through the authenticated backend invite-submission path, and assignment enters pending state only if pending-cap constraints pass, no same Student/Professional/Specialty pending or active Connection already exists, and the Professional still has the matching active Specialty.
