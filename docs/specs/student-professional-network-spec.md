...
11. Student submits invite code through the authenticated backend invite-submission path, and assignment enters pending state only if pending-cap constraints pass, no same Student/Professional/Specialty pending or active Connection already exists, and the Professional still has the matching active Specialty. The backend uses deterministic duplicate guards plus ten fixed pending-student slots per Professional so concurrent submissions cannot bypass those checks.
...
30. Nutrition plan search/lookup nutrient data is exposed through the MyChampions server `POST /integrations/food/search` route for MVP; the server owns the food-service provider boundary and local catalog Postgres mirror.
