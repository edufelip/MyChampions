...
- `FR-249A`: In local development, email/password sign-in and account creation shall use the MyChampions server email-auth boundary backed by local Postgres `local_email_auth_credentials` by default; when account creation returns the privacy-preserving session-less acknowledgement, the client shall complete authentication through the same server email sign-in boundary; the deterministic local dev-session endpoint must not be used for non-dev app variants.
