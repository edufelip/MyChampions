...
- `FR-121`: Assignment activation requires a confirmation step from the professional after the student submits the invite code. During the local server migration, local bearer-auth sessions confirm pending assignments through the MyChampions server Postgres endpoint; missing local server auth fails closed outside E2E fixtures.
...
- `FR-130`: A Student or Professional can end (unbind) their active relationship at any moment. During the local server migration, local bearer-auth sessions end the connection row through the MyChampions server Postgres endpoint; missing local server auth fails closed outside E2E fixtures. Plan archive/restore and tracking-access side effects are server-owned for local bearer sessions.
...
- `FR-134`: MVP nutrition data lookup shall be exposed to the mobile app through the MyChampions server `POST /integrations/food/search` route. The server owns the food-service credentials and reads local development results from the mirrored catalog Postgres database.
...
- `FR-179`: Professional invite codes shall be persistent (non-expiring by default), revocable/regenerable by professional action, scoped per Specialty, and stored by the active relationship backend. During the local server migration, local bearer-auth sessions use MyChampions server Postgres invite-code endpoints, and missing local server auth fails closed outside explicit E2E fixtures. Regenerating a code invalidates that Specialty code and auto-cancels pending requests created from it.
...
- `FR-222`: Effective hydration target shall resolve from the active nutrition plan context (assigned plan goal first; otherwise self-managed plan goal) through the MyChampions server hydration-goal context endpoint.
...
- `FR-231`: The compressed image shall be sent as base64 to the MyChampions server analyzer endpoint; the server calls the configured meal-photo analyzer provider and returns structured macro estimates (calories, carbs, proteins, fats, totalGrams, confidence).
...
- `FR-237`: The meal-photo analyzer provider API key must never be embedded in the client binary or exposed via client-accessible environment variables; all AI analysis calls must route through the MyChampions server, which validates the caller's bearer session before invoking the provider.
...
- `FR-249A`: In local development, email/password sign-in and account creation shall use the MyChampions server email-auth boundary backed by local Postgres `local_email_auth_credentials` by default; the deterministic local dev-session endpoint must not be used for non-dev app variants.
...
- `FR-251`: For email/password accounts, the "Change password" action shall submit a MyChampions server password-reset request for the user's registered email address, preceded by a user confirmation alert.
...
- `FR-254`: SC-208 exercise search/detail requests shall route through the exercise catalog microservice (`POST /catalog/search`, `GET /catalog/exercises/:id`) instead of direct upstream client calls.
...
- `FR-271`: Pull-request UI testing shall resolve changed paths against the target branch merge base, select affected Detox and Playwright suites from explicit feature ownership plus reverse dependency data, widen cross-cutting changes to every applicable platform suite, and fail closed to the complete relevant matrix when impact cannot be determined safely. Each selected Detox phase shall own a fresh Metro process, suppress only the in-app development LogBox overlay while preserving terminal diagnostics, terminate and verify every runner-owned member of that process group, and fail cleanup if a runner-owned member survives forced termination or the Metro port remains occupied. Android instrumentation shall route the React Native debug-server host through the configured ADB reverse port before launching the app, and shall fail before execution if that route cannot be persisted.
